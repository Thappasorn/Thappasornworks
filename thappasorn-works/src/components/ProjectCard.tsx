...
export default function ProjectCard({ p, vertical = false }: { p: Project; vertical?: boolean }) {
  return (
    <Link href={`/project/${p.slug}`} className={`group relative block overflow-hidden rounded-[14px] shadow-2xl transition-transform duration-500 ease-apple hover:-translate-y-1.5 ${vertical ? "aspect-[9/16]" : "aspect-[4/3]"}`}>
      {p.thumbnail ? (
