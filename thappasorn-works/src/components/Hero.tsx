...
export default function Hero({ showreelUrl, hero, ownerImage }: { showreelUrl?: string | null; hero?: HeroContent | null; ownerImage?: string | null }) {
  ...
  // override if the admin entered text for this language, else the translation file
  const tx = (k: "roles" | "l1" | "l2" | "l3" | "lead" | "cta1" | "cta2") => {
    const v = (ov as Record<string, string | undefined>)[k];
    return v && v.trim() ? v : t(k);
  };
